...
function Install-Desktop {
    ...
    try {
        ...
        # Workspace overrides (root package.json) already handle this; skip the manual pin
        # in workspace context to avoid npm EOVERRIDE (npm throws when you try to install a
        # package as a direct dep that is already pinned via root overrides).
        $rootPkgPath = Join-Path $InstallDir "package.json"
        ...
        if ($hasWorkspaceOverrides) {
            Write-Success "Desktop UI compatibility deps already pinned via workspace overrides"
        } else {
            Write-Info "Pinning desktop UI compatibility deps (@assistant-ui/store 0.2.9, @assistant-ui/tap 0.5.10)..."
            $pinPrevEAP = $ErrorActionPreference
            $ErrorActionPreference = "Continue"
            ...
            $ErrorActionPreference = $pinPrevEAP
            if ($pinCode -ne 0) {
                Write-Warn "Could not apply manual pin; proceeding with installed versions"
            } else {
