...
class TestRunBackgroundTask:
    ...
    async def test_media_files_routed_by_type(self, monkeypatch):
        ...
        try:
            ...
            assert os.path.realpath(mock_adapter.send_voice.call_args.kwargs["audio_path"]) == os.path.realpath(_ogg)
            mock_adapter.send_video.assert_called_once()
            assert os.path.realpath(mock_adapter.send_video.call_args.kwargs["video_path"]) == os.path.realpath(_mp4)
            mock_adapter.send_image_file.assert_called_once()
            assert os.path.realpath(mock_adapter.send_image_file.call_args.kwargs["image_path"]) == os.path.realpath(_png)
            mock_adapter.send_document.assert_called_once()
            assert os.path.realpath(mock_adapter.send_document.call_args.kwargs["file_path"]) == os.path.realpath(_pdf)
        finally:
